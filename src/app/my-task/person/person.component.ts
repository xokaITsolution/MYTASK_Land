...
this.getcustomerworkfrom()
  ...
  getCustomerStatusLookUP() {
    this.serviceService.getCustomerStatusLookUP().subscribe(CustomerStatusLookUP => {
      this.CustomerStatusLookUP = CustomerStatusLookUP;
      this.CustomerStatusLookUP = (Object.assign([], this.CustomerStatusLookUP.list));
  ...
  getCountryLookUP() {
    this.serviceService.getCountryLookUP().subscribe(CountryLookUP => {
  ...
  getCustomerLookUP() {
    this.serviceService.getCustomerLookUP().subscribe(CustomerLookUP => {
      ...
      for (let i = 0; i < this.CustomerLookUP.length; i++) {
        this.CustomerLookUP[i].FullName_AM = this.CustomerLookUP[i].Applicant_First_Name_AM + ' ' +
          this.CustomerLookUP[i].Applicant_Middle_Name_AM + ' ' + this.CustomerLookUP[i].Applicant_Last_Name_AM;
        this.CustomerLookUP[i].FullName_EN = this.CustomerLookUP[i].Applicant_First_Name_EN + ' ' +
          this.CustomerLookUP[i].Applicant_Middle_Name_En + ' ' + this.CustomerLookUP[i].Applicant_Last_Name_EN;
      }
        ...
        (response) => {
          if(Array.isArray(response)){          
            if(response.length > 0){               
              ...
              this.serviceService.getcostemerbyuserid(this.workingUser.UserId).subscribe((response:any) => {
                console.log('resppp',response)
                if(response.procCCustomerLoadAlls.length>0){
                  this.serviceService.getCustomer(response.procCCustomerLoadAlls[0].customer_ID).subscribe((responses:any) => {
                    console.log('respp',responses.list[0])
                    this.person=responses.list[0]
                    this.person.parent_Customer_ID
                    this.serviceService.getView_CustomerForWhereWhereWork().subscribe(res => {
                      this.customerworkfroms = res; 
                      this.customerworkfroms=  this.customerworkfroms.filter(x=>x.customer_ID== this.person.parent_Customer_ID)
                      console.log('customerworkfrom',this.customerworkfrom )
                      this.Parentname=this.customerworkfroms[0].applicant_First_Name_AM
                    }, error => {
                      console.log('error');
                    });
  ...
  getWoredaLookUP(spd) {
    this.serviceService.getWoredaLookUPbyorg(spd).subscribe(WoredaLookUP => {
  ...
  Save2() {
    ...
    this.serviceService.saveCustomer(this.person).subscribe(CustID => {
  ...
  update() {
    ...
    this.serviceService.UpdateCustomer(this.person).subscribe(CustID => {
      this.getcostumerbyid(this.person.customer_ID)
      const toast = this.notificationsService.success('Sucess', 'Succesfully edited');
