...
constructor(private ngxSmartModalService: NgxSmartModalService, private myTaskService: MyTaskService,
...
console.log('this.SuperviedUsers', this.SuperviedUsers);
  ...
  this.myTaskService.getMytasksbyusername(this.SuperviedUsers[i].UserName).subscribe(tasks => {
    ...
    if (i === this.SuperviedUsers.length - 1) {
      ...
      this.taskList=this.taskList.filter(x=>x.status != 'O' && x.status != 'C')
      console.log('this.taskList', this.taskList);
...
passdata(modal){
  this.username = modal.target.options[modal.target.options.selectedIndex].text;

  this.uid=modal.target.value
 }
assign(modal) {
 
  this.myTaskService.AssignToUser(this.selectedTask.todo_comment,this.uid,this.selectedTask.id
    ,this.username,'O',).subscribe(message => {
      if (message) {
        const toast = this.notificationsService.success('Sucess', 'assigned sucessfully');
      } else {
        const toast = this.notificationsService.error('Error', 'Assign falied');
      }
    },
    error => {
      console.log(error);
