...
export class PropertyRegisterComponent implements OnInit, OnChanges {
  ...
  async ngOnChanges() {
    ...
    if (this.propertyRegister.registration_Date) {
      ...
      this.propertyRegister.registration_Date=await this.getgregorianToEthiopianDate(this.propertyRegister.registration_Date)}
    }
  ...
  selectedDateTime(dates: any,selecter){
    if(selecter == 1){
        ...
        this.propertyRegister.registration_Date= dates[0]._day +"/"+ dates[0]._month +"/"+ dates[0]._year
  ...
  async save() {
    ...
    prop.registration_Date=await this.getEthiopianToGregorian(prop.registration_Date)
  ...
  getproploc(plotid){
    this.serviceService.getProploc(plotid).subscribe(
      (response:any) => {
...
if(this.proploc.length>0){
 ...
 this.isproplocnew=false}
  ...
  getproplocbyid(){
    this.serviceService.getPlotloc(this.selectedpro.plot_ID).subscribe(
      (response:any) => {
...
if(this.plotloc.length>0){
  ...
  console.log('plotlocccc:',this.plotloc,this.plotloc[0].geowithzone);
  this.serviceService.getProploc(this.selectedpro.property_ID).subscribe(
    (response:any) => {
this.proploc=response.procProporty_Locations
if(this.proploc.length>0){
this.propformLocation=this.proploc[0]
...
console.log('protlocprotlocprotlocprotloc:',this.proploc,this.proploc[0].geowithzone);
this.isproplocnew=false}
else{
 this.isproplocnew=true
}
  ...
  convertToMultiPoint(points: { easting: number, northing: number ,hemisphere:string,zone:number}[]): string {
    const multiPointArray = points.map(point => `${point.easting} ${point.northing} ${point.hemisphere} ${point.zone}`).join(', ');
  ...
  convertToMultiPoints(points: { easting: number, northing: number }[]): string {
    const multiPointArray = points.map(point => `${point.easting} ${point.northing}`).join(', ');
  ...
  convertCoordinates(data) {
    ...
    console.log('convertedCconvertedCoordinatesoordinates', convertedCoordinates)
  }
  updateproploc(){
      ...
      this.serviceService.getUserRole().subscribe(
        (response:any) => {
          ...
          if(response[0].RoleId=="f8dda85e-f967-4ac5-bf79-4d989ecfb863"){
            ...
            console.log(this.propformLocation.team_Leader_Approved_By,this.propformLocation.team_Leader_Approved)
          }
          else if(response[0].RoleId=="fe7be2e0-e717-4230-b732-5b810a8bb875"){
            this.propformLocation.baseMap_Approved_By=response[0].UserId
            this.propformLocation.baseMap_Approved=true
            console.log(this.propformLocation.team_Leader_Approved_By,this.propformLocation.team_Leader_Approved)
          }
          this.serviceService.updateProploc(this.propformLocation).subscribe(CustID => {
     
      this.getproploc( this.propformLocation.proporty_Id)
      const toast = this.notificationsService.success('Sucess', 'Succesfully Upadted');
    },
    error => {
      console.log('error');
      const toast = this.notificationsService.error('error',`unable to Update ${error['status'] == 0 ? error['message'] : JSON.stringify(JSON.stringify(error['error']))}`);}
)})}
  }
  saveproploc(){
    console.log('coordinatcoordinat',this.serviceService.coordinate)
    ...
    this.serviceService.getUserRole().subscribe(
      (response:any) => {
        let coordinates= this.convertToMultiPoints(this.serviceService.coordinate)
    ...
    this.serviceService.saveProploc(this.propformLocation).subscribe(CustID => {
      this.getproploc( this.propformLocation.proporty_Id)
      const toast = this.notificationsService.success('Sucess', 'Succesfully saved');
      this.serviceService.hide=false
    },
    error => {
      console.log('error');
      const toast = this.notificationsService.error('error',`unable to Save ${error['status'] == 0 ? error['message'] : JSON.stringify(JSON.stringify(error['error']))}`);}
)})}
  }
  Uploader(File) {
    let base64file;
