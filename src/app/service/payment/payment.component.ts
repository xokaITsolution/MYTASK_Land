...
Amount;
  ...
  constructor(
    ...
    private serviceService: ServiceService, private serviceComponent: ServiceComponent,
 ...
 ngOnChanges() {
  ...
  this.PaymentProcessPath = this.sanitizer.bypassSecurityTrustResourceUrl(environment.PaymentReportPath +
    '/?UserName=' + environment.username + '&ApplicatioNo=' + this.AppNo);
}
  ...
  getPaymentManagement() {
    ...
    this.serviceService.getPayment(this.AppNo).subscribe(PaymentList => {
...
isNew:boolean=false;
  SelectPayment(Payment) {
    ...
    this.serviceService.getPaymentDetail(Payment.PID).subscribe(PaymentDetailList => {
      ...
      for (let i = 0; i <= this.PaymentDetailList.length; i++) {
        if (this.PaymentDetailList[i].Is_Paid) {
          this.totalPrice = this.totalPrice + this.PaymentDetailList[i].Price;
        }
      }
    }, error => {
      console.log('error');
 ...
 console.log("PaymentDetail",PaymentDetail)
    this.serviceService.addPaymentDetail(PaymentDetail).subscribe(message => {
      const toast = this.notificationsService.success('Sucess', message);
  ...
  SavePayment(Payment) {
    ...
    this.serviceService.savePayment(Payment).subscribe(message => {
      const toast = this.notificationsService.success('Sucess', message);
